...
async def send_card_to_user(call: types.CallbackQuery, state: FSMContext):
    if call.data.isdigit():
        ...
        await dp.bot.send_message(chat_id=int(call.data), text=f"😊 Iltimos kuting admin sizga karta tashlamoqda ...",
        ...
        await update_buy(random_number=data[1])
        filial = f""
        for i in await get_about_product(random_number=data[1]):
            filial = f"{i['which_filial']}"
        user = await get_user(chat_id=data[0])
        if user[3] == "uz":
            await dp.bot.send_message(chat_id=data[0],
        ...
        await call.message.answer(text=f"✅ Foydalanuvchiga buyurtma tayyorligi haqida habar yuborildi.", reply_markup=admins_panel)
